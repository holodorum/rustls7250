...
mod test_raw_keys {
    ...
    use rustls::{derive_traffic_iv, derive_traffic_key};
    use rustls::{Connection, Tls13CipherSuite};

    #[test]
    fn succesful_raw_key_connection_and_correct_peer_certificates() {
        for kt in ALL_KEY_TYPES {
            let client_config = make_client_config_with_raw_key_support(kt.clone());
            let server_config = make_server_config_with_raw_key_support(kt.clone());
            ...
            match client.peer_certificates() {
                ...
                None => {
                    assert!(false, "Client should have received a certificate")
                }
            }
            ...
            match server.peer_certificates() {
                ...
                None => {
                    assert!(false, "Server should have received a certificate")
                }
            }
        }
    }
    ...
    fn only_client_supports_raw_keys() {
        for kt in ALL_KEY_TYPES {
            let client_config_rpk = make_client_config_with_raw_key_support(kt.clone());
            let server_config = make_server_config(kt.clone());
            ...
            match do_handshake_until_error(&mut client_rpk, &mut server) {
                ...
                _ => {
                    assert!(
                        false,
                        "Expected error because client is incorrectly configured"
                    )
                }
            }
        }
    ...
    fn only_server_supports_raw_keys() {
        for kt in ALL_KEY_TYPES {
            let client_config =
                make_client_config_with_versions(kt.clone(), &[&rustls::version::TLS13]);
            let server_config_rpk = make_server_config_with_raw_key_support(kt.clone());
    ...
    fn incorrectly_alter_client_hello() {
        for kt in ALL_KEY_TYPES {
            let client_config = Arc::new(make_client_config(kt.clone()));
            let server_config_rpk = Arc::new(make_server_config_with_raw_key_support(kt.clone()));
            ...
            match server_cert_altered {
                Ok(_) => assert!(false, "Expected error because server cert is altered"),
                Err(err) => assert_eq!(
                    err,
                    ErrorFromPeer::Server(Error::PeerIncompatible(
                        PeerIncompatible::IncorrectCertificateTypeExtension
    ...
    fn incorrectly_alter_server_hello() {
        for kt in ALL_KEY_TYPES {
            ...
            let client_config = make_client_config(kt.clone());
            let server_config_rpk = make_server_config(kt.clone());
            add_keylog_and_do_altered_handshake(
                client_config,
                server_config_rpk,
                supported_suite,
    ...
    fn alter_server_hello_message(
        ...
        let encoded = if let MessagePayload::Handshake { parsed, .. } = &mut msg.payload {
            if let HandshakePayload::EncryptedExtensions(enc_ext) = &mut parsed.payload {
                ...
                if !sct_present && server_cert_type.is_some() {
                    enc_ext.push(ServerExtension::ServerCertType(server_cert_type.unwrap().clone()));
                }
                if !cct_present && client_cert_type.is_some() {
                    enc_ext.push(ServerExtension::ClientCertType(client_cert_type.unwrap().clone()));
                }
            }
    ...
    fn derive_message_encrypter_and_decrypter(
        ...
        // Derive Encrypter
        let key = derive_traffic_key(expander.as_ref(), cipher_suite.aead_alg.key_len());
