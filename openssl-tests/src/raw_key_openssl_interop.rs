...
mod client {
    ...
    pub(super) fn make_config(client_private_key: &str, server_pub_key: &str) -> ClientConfig {
        ...
        let certified_key: Arc<CertifiedKey> = Arc::new(CertifiedKey::new(
            vec![client_public_key_as_cert],
            client_private_key,
        ));

        let server_cert_verifier = Arc::new(SimpleRpkServerCertVerifier::new(vec![server_raw_key]));
        let client_cert_resolver = Arc::new(AlwaysResolvesClientRawPublicKeys(certified_key));
...
mod server {
    ...
    pub(super) fn make_config(server_private_key: &str, client_pub_key: &str) -> ServerConfig {
        ...
        let server_cert_resolver = Arc::new(AlwaysResolvesServerRawPublicKeys(certified_key));
